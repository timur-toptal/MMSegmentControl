...
open class MMSegmentedControl: UIControl {
    open weak var delegate: MMSegmentedControlDelegate?

    open var layoutPolicy: MMSegmentedControlLayoutPolicy = .fixed
    ...
    /// Only available in dynamic layout policy
    open var selectionHorizontalPadding: CGFloat = SegmentConstant.defaultSelectionHorizontalPadding
    ...
    open fileprivate(set) var style: MMSegmentedControlStyle = .text

    open var selectionBoxStyle: MMSegmentedControlSelectionBoxStyle = .none
    ...
    open var isLongPressEnabled = false {
        didSet {
            if isLongPressEnabled {
                ...
                longPressGesture!.addTarget(self, action: #selector(MMSegmentedControlLongPressed(_:)))
    ...
    open class func initWithTitles(_ titles: [NSAttributedString], selectedTitles: [NSAttributedString]?) -> MMSegmentedControl {
        let segmentedControl = MMSegmentedControl(frame: CGRect.zero)
    ...
    open class func initWithImages(_ images: [UIImage], selectedImages: [UIImage]?) -> MMSegmentedControl {
        let segmentedControl = MMSegmentedControl(frame: CGRect.zero)
    ...
    open override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if 0..<segmentsCount() ~= touchIndex {
            delegate?.MMSegmentedControl(self, didSelectIndex: touchIndex)
...
public extension MMSegmentedControl {
    // MARK: - Events
...
extension MMSegmentedControl: UIGestureRecognizerDelegate {
    ...
    fileprivate func longPressDidBegin(_ gesture: UIGestureRecognizer) {
        ...
        if 0..<segmentsCount() ~= longPressIndex {
            delegate?.MMSegmentedControl(self, didLongPressIndex: longPressIndex)
        }
    }
}
